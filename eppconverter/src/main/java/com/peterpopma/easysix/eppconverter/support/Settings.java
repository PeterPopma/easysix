package com.peterpopma.easysix.eppconverter.support;
...
public final class Settings {
    ...
    public static void init(){
		...
		try {
			...
			log.error("Het socket adapter properties bestand kan niet gevonden worden: \"" + INSTELLINGEN_PROPERTIES + "\"");
		} catch (IOException e) {
			log.error("Fout tijdens het laden van de socket adapter properties: \"" + INSTELLINGEN_PROPERTIES + "\"");
		}catch(ConfiguratieException e){
			log.error("Fout tijdens het laden van de socket adapter property", e);
		}
	...
	private static void laadInstellingen(){
		log.info("Start laden configuratie");
		...
		log.info("Klaar met laden configuratie");
	}
